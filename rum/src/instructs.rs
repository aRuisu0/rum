...
pub fn cond_move(vm: &mut Vm, word: u32) {
	let (a, b, c) = three_registers(word);

    // Use a match expression to check the value of the c register.
    match vm.registers[c] {
        // If the c register is not zero, set the value of the a register to the value of the b register.
        0 => (),
        _ => vm.registers[a] = vm.registers[b],
    }
}
